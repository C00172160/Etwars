...
Play::Play(Game* game, int selectedMap, bool VSYNC, bool FULLSCREEN, bool AUDIO) : World(b2Vec2(0.0f, 10.0f))
{
	...
	captain1HealthSprite.setTexture(healthTex);
	captain2HealthSprite.setTexture(healthTex);
	...
	water4.setPositon(sf::Vector2f(2121, 555));
	water5.setPositon(sf::Vector2f(2828, 555));
	water6.setPositon(sf::Vector2f(3535, 555));
	water7.setPositon(sf::Vector2f(-707, 555));
	water8.setPositon(sf::Vector2f(-1414, 555));
	water9.setPositon(sf::Vector2f(-2121, 555));
	...
	placingspriterectangle.setFillColor(sf::Color::Black);
	...
	background2.setOrigin(backGroundTexture.getSize().x / 2, backGroundTexture.getSize().y / 2);
	background3.setOrigin(backGroundTexture.getSize().x / 2, backGroundTexture.getSize().y / 2);
	background.setPosition(sf::Vector2f(990, 40));
	background.setScale(1, 1);
	background2.setPosition(sf::Vector2f(background.getPosition().x - background.getGlobalBounds().width, 40));
	background2.setScale(-1, 1);
	background3.setPosition(sf::Vector2f(background.getPosition().x + background.getGlobalBounds().width, 40));
	...
	topSprite.setOrigin(top.getSize().x / 2, top.getSize().y / 2);
	topSprite2.setOrigin(top.getSize().x / 2, top.getSize().y / 2);
	topSprite3.setOrigin(top.getSize().x / 2, top.getSize().y / 2);
	topSprite.setPosition(sf::Vector2f(990, background.getPosition().y - background.getGlobalBounds().height/2 - 305));
	topSprite2.setPosition(sf::Vector2f(topSprite.getPosition().x - topSprite.getGlobalBounds().width, background.getPosition().y - background.getGlobalBounds().height / 2 - 305));
...
void Play::update()
{
	...
	game->window.draw(background2);
	game->window.draw(background3);
...
//	game->window.draw(system);
	...
	if (BuildMode == false)
	{
		...
		if (Player1Turn == true)
		{
			...
			cross.Update(player1team[player1Number].getPosition(),player1team[player1Number].getType(),player1team[player1Number],Power);
		...
		else
		{
			...
			turnTimer -= turnCLock.restart().asSeconds();
			cross.Update(player2team[player2Number].getPosition(), player2team[player2Number].getType(), player1team[player1Number],Power);
	...
	if (BuildMode == false)
	{
		float captain1health = captain1.getHealth();
		captain1HealthSprite.setScale(captain1health / 100, 1);
		captain1HealthSprite.setPosition(captain1.getPosition().x - 10, captain1.getPosition().y - 40);

		float captain2health = captain2.getHealth();
		captain2HealthSprite.setScale(captain2health / 100, 1);
		captain2HealthSprite.setPosition(captain2.getPosition().x - 10, captain2.getPosition().y - 40);

	

		if (captain2.getHealth() > 0)
		{
			game->window.draw(captain2HealthSprite);
		}
		if (captain1.getHealth() > 0)
		{
			game->window.draw(captain1HealthSprite);
		}
			...
			if (event.key.code == sf::Keyboard::E)
			{
				if (BuildMode == false)
				{
					if (Player1Turn == true)
					{
						if (CountDown == false)
						{
							if (player1team[player1Number].getType() == 1 && Rockets.size() == 0 && buttonpressed == true)
							{
								player1Fire = true;
								Rocket tempRocket(World, cross.getPosition(), RocketTexture, player1team[player1Number].getPosition(), Power);
					...
					if (BuildMode == false)
					{
						...
						{
							if (player2team[player2Number].getType() == 1 && Rockets.size() == 0 && buttonpressed == true)
							{

								player2Fire = true;
								Rocket tempRocket(World, cross.getPosition(), RocketTexture, player2team[player2Number].getPosition(), Power);
								Rockets.push_back(tempRocket);
								if (effectToggle == true)
								{
									soundManager.playFireSound();
						...
						else
						{
							if (CountDown == false)
							{
								if (player2team[player2Number].getType() == 1 && Rockets.size() == 0 )
								{
								 ...
								 if (player2team[player2Number].getType() == 2 && Handguns.size() == 0)
								{
									player2Fire = true;
...
void Play::BuildModeUpdate(){
	...
	hudSpriterect.setSize(sf::Vector2f(HudSprite.getGlobalBounds().width, HudSprite.getGlobalBounds().height));
	hudSpriterect.setPosition(HudSprite.getPosition());

	for (int j = 0; j < blocks.size(); j++)
	{
		if (abs(placingSprite.getPosition().x - blocks[j].getBody()->GetPosition().x *SCALE) < 50 && abs(placingSprite.getPosition().y - blocks[j].getBody()->GetPosition().y*SCALE) < 50)
		{
			if (CollisionManager::RectangleRectangleCollision(placingspriterectangle, blocks[j].getRect()) == true )//|| CollisionManager::RectangleRectangleCollision(placingspriterectangle, hudSpriterect))
			{
				placeable = false;
				break;
			}
	...
	placingspriterectangle.setOrigin(placingSprite.getGlobalBounds().width / 2, placingSprite.getGlobalBounds().height / 2);
	placingspriterectangle.setSize(sf::Vector2f(placingSprite.getGlobalBounds().width, placingSprite.getGlobalBounds().height));
	placingspriterectangle.setPosition(sf::Vector2f(position.x,position.y));
...
void Play::UpdateBlocks()
{
	...
	{	
		...
		if (CollisionManager::CircleRectangleCollision(boundingbox, captain1.getPlayerRectangle()) == true)
		{
			captain1.setHealth(50);
		}
		if (CollisionManager::CircleRectangleCollision(boundingbox, captain2.getPlayerRectangle()) == true)
		{
			captain2.setHealth(50);
		}
