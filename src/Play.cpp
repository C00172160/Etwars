...
void Play::checkAlive(){
	...
	for (int i = 0; i < player1team.size(); i++)
	{
		if (player1team[i].getHealth() <= 0 &&  World.IsLocked() == false)
		{
			World.DestroyBody(player1team[i].getBody());
			player1team.erase(player1team.begin() + i);
		}
	...
	//	{
	//		World.DestroyBody(player2team[i].getBody());
	//		player2team.erase(player2team.begin() + i);
	//	}
